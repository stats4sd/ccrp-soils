SELECT
        `samples`.`project_id` AS `project_id`,`samples`.`identifiers`->"$.county_id" as `county_id`,`samples`.`identifiers`->"$.group_id" as `group_id`,`samples`.`identifiers`->"$.ngo_id" as `ngo_id`,`samples`.`identifiers`->"$.farmer_id" as `farmer_id`,`samples`.`identifiers`->"$.anchor_plot_good" as `anchor_plot_good`,
        ...
        IFNULL(IFNULL(IFNULL(CONCAT(`samples`.`final_texture_type_usda`, ',', IFNULL(`samples`.`second_texture_type_usda`, '')), CONCAT(`samples`.`final_texture_type_fao`, ',', IFNULL(`samples`.`second_texture_type_fao`, ''))), `samples`.`simple_texture`), 'NOT YET RATED') AS `soil_texture`,
